  ...
  const handleAddMember = async (newMember) => {
    try {
      ...
      if (error.response && error.response.status === 409) {
        toast.error('Email Already Exists'); // Display the error message from the response
      } else if (error.response && error.response.status === 400) {
        toast.error('Flat Number Already Exists'); // Display the error message from the response
      } else {
        console.error(error);
        toast.error('An error occurred. Please try again.'); // Generic error message for other errors
      }
    }
  };
      ...
      if(isNameValid(editedMember.firstName)) {
        if(isNameValid(editedMember.lastName)) {
          if(isMobileNumberValid(editedMember.mobileNo)) {
            ...
            toast.error('Member Updated Successfully!');
            closeEditModal();
            fetchMembersData();
          } else {
          ...
          toast.error("Last Name should contain only Letters and White Spaces!");
        }
      } else {
        toast.error("First Name should contain only Letters and White Spaces!");
      }
    } catch (error) {
      console.error(error);
      toast.error('An error occurred. Please try again.'); // Generic error message for other errors
