    ...
    const eleid = localStorage.getItem('election_id');
      try{
        ...
        {headers: {'Content-Type': 'application/json'}})
          toast.success("Nominations Posted into Ballot box Successfully!");
      } catch(error){
  ...
  async function handleClick4() {
    const eleid = localStorage.getItem('election_id');
      try{
        ...
        {headers: {'Content-Type': 'application/json'}})
          toast.success("Votes counted and posted into ballot box for each contestant Successfully!");
      } catch(error){
          toast.error(JSON.stringify(error));
      }
  };
    ...
    const eleid = localStorage.getItem('election_id');
      try{
        const response = await axios.post(`http://localhost:9001/results/fetchresults/${eleid}`)
          toast.success("Data Posted into results Successfully!");
      } catch(error){
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (error.response && error.response.status === 409) {
        toast.error('An election is already registered on the given date!');
      } else {
        console.error(error);
